...
namespace LD3
{
    class Operatorius
    {
        ...
        public static bool operator <=(Operatorius op1, Operatorius op2)
        {
            ...
            return (p < 0 || (p == 0 && op1.pradSum > op2.pradSum));
        }
    ...
    class Korteles
    {
        ...
        public void Nemokamai()
        {
            for (int i = 0; i < n; i++)
                if (Op[i].KoksTarifasISavusTinklus() > 0 && Op[i].SMSiSavus() > 0)
                {
                    for (int j = i; j < n - 1; j++)
                        Op[j] = Op[j + 1];
                    n = n - 1;
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            korteles.Nemokamai();
            Spausdinti(rez, korteles, "Paliktos tik tos kortelės, su kuriomis į savus tinklus galima skambinti ir rašyti nemokamai:");

            korteles.Sort();
            Spausdinti(rez, korteles, "Surikiuotos kortelės:");
