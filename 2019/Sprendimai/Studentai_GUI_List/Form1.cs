...
namespace Studentai_GUI_List
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            lenteleToolStripMenuItem.Enabled = false;
            studentoĮvertinimaiToolStripMenuItem.Enabled = false;
            studentųSkaičiusToolStripMenuItem.Enabled = false;
            vaikinųVidurkisToolStripMenuItem.Enabled = false;
            merginųVidurkisToolStripMenuItem.Enabled = false;
        ...
        private void įvestiToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK) // jeigu pasirinktas failas
            {
                ...
                lenteleToolStripMenuItem.Enabled = true;
                studentųSkaičiusToolStripMenuItem.Enabled = true;
                studentoĮvertinimaiToolStripMenuItem.Enabled = true;
                vaikinųVidurkisToolStripMenuItem.Enabled = true;
                merginųVidurkisToolStripMenuItem.Enabled = true;
            }
        }
        ...
        private void spausdintiToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                ...
                dataGridView1.Columns[3].Name = "Lytis";
                dataGridView1.Columns[3].Width = 80;
                for (int i = 0; i < StudentuTestas.Count; i++)
                {
                    Studentas studentas = StudentuTestas[i];
                    dataGridView1.Rows.Add(i + 1, studentas.PavVrd, studentas.Pazym, studentas.Lytis);
                }
            }
        }
        ...
        private void lenteleToolStripMenuItem_Click(object sender, EventArgs e)
        {
            OpenFileDialog openNewFile = new OpenFileDialog();
            openFileDialog1.Filter = "txt files (*.txt)|*.txt|All files (*.*)|*.*";
            openFileDialog1.Title = "Pasirinkite failą, iš kurio spausdinsite";
            DialogResult result = openNewFile.ShowDialog();
            if (result == DialogResult.OK)
            {
        ...
        private bool arYra(string lyt)
        {
            bool yra = false;
            foreach (Studentas st in StudentuTestas)
            {
                if (st.Lytis == lyt)
                {
                    yra = true;
        ...
        private void vaikinųVidurkisToolStripMenuItem_Click(object sender, EventArgs e)
        {
            bool yra = arYra("v");
            if (yra)
            {
                MessageBox.Show(string.Format("Vaikinų vidurkis: {0, 5:f}", skaiciuotiVidurki("v")));
            }
            else
            {
                MessageBox.Show("Vaikinų fakultete nėra");
            }
        }

        private void merginųVidurkisToolStripMenuItem_Click(object sender, EventArgs e)
        {
            bool yra = arYra("m");
            if (yra)
            {
                MessageBox.Show(string.Format("Merginų vidurkis: {0, 5:f}", skaiciuotiVidurki("m")));
            }
            else
            {
                MessageBox.Show("Merginų fakultete nėra");
            }
        }

        private double skaiciuotiVidurki(string lyt)
        {
            double suma = 0;
            int kiekis = 0;
            foreach (Studentas st in StudentuTestas)
        ...
        static List<Studentas> SkaitytiStudList(string fv)
        {
            ...
            using (StreamReader srautas = new StreamReader(fv, Encoding.GetEncoding(1257)))
            {
                ...
                while ((eilute = srautas.ReadLine()) != null)
                {
                    ...
                    string lyt = eilDalis[2].Trim();
                    Studentas studentas = new Studentas(pavVrd, pazym, lyt);
